...
import { PsychoJS } from 'https://pavlovia.org/lib/core-3.2.0.js';
import * as core from 'https://pavlovia.org/lib/core-3.2.0.js';
import { TrialHandler } from 'https://pavlovia.org/lib/data-3.2.0.js';
import { Scheduler } from 'https://pavlovia.org/lib/util-3.2.0.js';
import * as util from 'https://pavlovia.org/lib/util-3.2.0.js';
import * as visual from 'https://pavlovia.org/lib/visual-3.2.0.js';
import { Sound } from 'https://pavlovia.org/lib/sound-3.2.0.js';
...
let expName = 'openIAT2';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  instruct_done = new core.Keyboard({psychoJS, clock: new util.Clock(), waitForStart: true});
  ...
  button_L = new visual.Rect ({
    win: psychoJS.window, name: 'button_L',
    units: 'height',
  ...
  button_R = new visual.Rect ({
    win: psychoJS.window, name: 'button_R',
    units: 'height',
    width: [0.4, 0.2][0], height: [0.4, 0.2][1],
    ori: 0, pos: [0.4, (- 0.3)],
    lineWidth: 1, lineColor: new util.Color('darkgreen'),
...
function instruct_pagesLoopBegin(thisScheduler) {
  ...
  for (const thisInstruct_page of instruct_pages) {
    thisScheduler.add(importConditions(instruct_pages));
...
function trialRoutineEachFrame() {
  ...
  // *button_left* updates
  if (t >= 0.0 && button_left.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
  ...
  if (t >= 0.0 && button_right.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
  ...
  if (t >= 0.0 && trial_label_right.status === PsychoJS.Status.NOT_STARTED) {
    // keep track of start time/frame for later
...
function end_thanksRoutineBegin() {
